...
class postgrey::config() {
  ...
  concat {
    "/etc/${postgrey::config_directory}/${wlc_basename}.local":
  }

  concat::fragment { 'postgrey/whitelist_clients.local/header':
     target => "/etc/${postgrey::config_directory}/${wlc_basename}.local",
     order  => 00,
  ...
  concat {
    "/etc/${postgrey::config_directory}/${wlr_basename}":
  }

  concat::fragment { 'postgrey/whitelist_recipients.local/header':
     target => "/etc/${postgrey::config_directory}/${wlr_basename}",
     order  => 00,
